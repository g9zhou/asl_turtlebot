...
class Ekf(object):
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        K = self.Sigma@H.T@scipy.linalg.inv(S)
        self.x += K@z
        self.Sigma -= K@S@K.T
...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        z = np.array([*v_list]).reshape(-1)
        Q = scipy.linalg.block_diag(*Q_list)
        H = np.array([*H_list]).reshape(-1,3)
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(I):
            ...
            H_temp = []
            for j in range(J):
                v = angle_diff(z_raw[:,i],hs[:,j])
                S = Hs[j]@self.Sigma@Hs[j].T+Q_raw[i]
                d = v.T@scipy.linalg.inv(S)@v
                if d < self.g**2:
                    v_temp.append(v)
                    Q_temp.append(Q_raw[i])
                    H_temp.append(Hs[j])
            if len(v_temp) > 0:
                k = np.argmin(scipy.linalg.norm(np.array(v_temp),axis=1))
                v_list.append(v_temp[k])
                Q_list.append(Q_temp[k])
                H_list.append(H_temp[k])
