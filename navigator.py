...
from planners import AStar, compute_smoothed_traj, FMTStar
...
class Navigator:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/exploration",Bool,self.exploration_callback)
        self.vis_pub = rospy.Publisher('marker_topic', Marker, queue_size = 500)

        print("finished init")

    def exploration_callback(self,msg):
        if msg and not self.explored: 
            rospy.sleep(10)
            self.explored = True
            self.start_pos_thresh = 2
            self.spline_alpha = 0.1
            self.time_gain = 1.35
            self.v_max = 1.5
            self.om_max = 0.6
    ...
    def dyn_cfg_callback(self, config, level):
        ...
        self.pose_controller.k2 = config["k2"]*3
        self.pose_controller.k3 = config["k3"]*3
        return config
    ...
    def replan(self):
        ...
        if not self.explored:
            # for i in range(20): print("using astar")
            problem = AStar(
                state_min,
                state_max,
                x_init,
            ...
            # for i in range(20): 
                # print("using FMTSTAR")
                # print(self.start_pos_thresh,self.time_gain,self.spline_alpha)
            problem = FMTStar.FMTStar(
                state_min,
                state_max,
                x_init,
                x_goal,
            ...
            for i, point in enumerate(problem.V):
                marker = Marker()
        ...
        if len(planned_path) < 4:
            ...
            return
        if self.explored and np.linalg.norm(np.array([self.x,self.y])-np.array([self.x_g,self.y_g])) < 0.5:
            self.switch_mode(Mode.PARK)
            return
        ...
        self.current_plan_duration = self.time_gain*t_new[-1]
    ...
    def run(self):
        ...
        # for i in range(10): print(self.start_pos_thresh,self.time_gain,self.spline_alpha)
