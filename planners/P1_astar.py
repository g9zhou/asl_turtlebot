...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        # self.obstacles = self.occupancy.obstacles
        # if DetOccupancyGrid2D.is_free(self,x) == False:
            # return False
        ...
        if x[0] > self.statespace_hi[0] or x[0] < self.statespace_lo[0]:
            return False
        if x[1] > self.statespace_hi[1] or x[1] < self.statespace_lo[1]:
            return False
        return True
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        neighbors.append(self.snap_to_grid((x[0],x[1]-self.resolution)))
        neighbors.append(self.snap_to_grid((x[0]+(self.resolution/np.sqrt(2)),x[1]+(self.resolution/np.sqrt(2)))))
        neighbors.append(self.snap_to_grid((x[0]-(self.resolution/np.sqrt(2)),x[1]+(self.resolution/np.sqrt(2)))))
        neighbors.append(self.snap_to_grid((x[0]+(self.resolution/np.sqrt(2)),x[1]-(self.resolution/np.sqrt(2)))))
        neighbors.append(self.snap_to_grid((x[0]-(self.resolution/np.sqrt(2)),x[1]-(self.resolution/np.sqrt(2)))))
        
        for nbr in neighbors:
            if self.is_free(nbr) == False:
                #neighbors.remove(nbr)
                neighbors = [value for value in neighbors if value != nbr]
        ########## Code ends here ##########
        return neighbors
        ...
        while len(self.open_set)>0:
            ...
            for x_neigh in self.get_neighbors(x_current):
                if x_neigh in self.closed_set:
                    continue
                tentative_cost_to_arrive = self.cost_to_arrive.get(x_current)+self.distance(x_current, x_neigh)
                if x_neigh not in self.open_set:
                    self.open_set.add(x_neigh)
                elif tentative_cost_to_arrive > self.cost_to_arrive.get(x_neigh):
                    continue
